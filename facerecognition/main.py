...
import os
import time  # 추가된 부분
from dlib import correlation_tracker  # 추가된 부분
...
db_path = 'C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\university.db' 

# 사전 훈련된 모델 경로 설정
predictor_model = 'C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\classified_model\\shape_predictor_68_face_landmarks.dat'
face_descriptor_model = 'C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\classified_model\\dlib_face_recognition_resnet_model_v1.dat'
model_path = 'C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\classified_model\\keras_face_recognition_model.keras'
scaler_path = 'C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\classified_model\\scaler.pkl'
pca_svm_model_path = 'C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\classified_model\\pca_svm_face_recognition_model.pkl'


# dlib의 얼굴 감지기 및 포즈 예측기 초기화
face_detector = dlib.get_frontal_face_detector()
face_pose_predictor = dlib.shape_predictor(r'C:\Users\82103\Downloads\PROJECT\project_facerecognition\classified_model\shape_predictor_68_face_landmarks.dat')
face_descriptor_extractor = dlib.face_recognition_model_v1(r'C:\Users\82103\Downloads\PROJECT\project_facerecognition\classified_model\dlib_face_recognition_resnet_model_v1.dat')
...
def generate_video_feed(capture_flag=False, num_images=15, output_dir='C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\capture_images',class_id=None):
...
def perform_attendance(class_id, week):
    image_folder = 'C:\\Users\\82103\\Downloads\\PROJECT\\project_facerecognition\\capture_images'
    ...
    if recognized_students:
        print(f"인식된 학생 ID: {recognized_students}")
    else:
        print("인식된 학생이 없습니다.")
...
def update_attendance(class_id):
    if 'loggedin' in session:
        ...
        cursor.execute('''
            UPDATE attendance
            SET status = ?
            WHERE student_id = ? AND class_id = ? AND strftime('%W', attendance_date) = strftime('%W', date('now'))
        ''', (status, student_id, class_id))
        print(f"{student_id} 학생의 출석 상태를 {status}로 기록.")
        ...
        return redirect(url_for('studentlist', class_id=class_id, week=request.args.get('week', 1)))
...
def studentlist(class_id, week):
    ...
    if 'loggedin' in session:
        ...
        for student in students:
            if student['attendance_date']:  # attendance_date가 None이 아닐 경우
